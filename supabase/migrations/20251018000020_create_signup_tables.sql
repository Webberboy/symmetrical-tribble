...
CREATE POLICY "Service role can manage roles" ON public.user_roles FOR ALL
    USING (auth.jwt() ->> 'role' = 'service_role');
...
CREATE POLICY "Service role can manage accounts" ON public.accounts FOR ALL
    USING (auth.jwt() ->> 'role' = 'service_role');

-- Pending signups policies
CREATE POLICY "Users can manage own pending signup" ON public.pending_signups FOR ALL
    USING (auth.uid() = auth_user_id);

CREATE POLICY "Service role can manage pending signups" ON public.pending_signups FOR ALL
    USING (auth.jwt() ->> 'role' = 'service_role');
